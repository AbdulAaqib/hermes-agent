...
CONFIG_SCHEMA = ProviderConfigSchema(
    ...
    fields=(
        ...
        _field("contextTokens", "Context token cap", KIND_NUMBER, "Cap on auto-injected context tokens. Blank uses the default (2000 on new installs).",
               placeholder="2000", group="Recall"),
        _field("searchTopK", "Search top-k", KIND_NUMBER, "Semantically relevant conclusions returned per context lookup. Blank uses the Honcho default.",
               placeholder="(server default)", group="Recall"),
        _field("searchMaxDistance", "Search max distance", KIND_NUMBER, "Maximum semantic distance (0.0-1.0) for context search results. Blank uses the Honcho default.",
               placeholder="(server default)", group="Recall"),
        _field("maxConclusions", "Max conclusions", KIND_NUMBER, "Maximum conclusions included in a representation per context lookup. Blank uses the Honcho default.",
               placeholder="(server default)", group="Recall"),
        # — Summarization & dreams —
        _field("summaryEnabled", "Session summaries", KIND_BOOL, "Let Honcho summarize sessions. Blank uses the server default (on).",
               group="Summarization"),
        _field("messagesPerShortSummary", "Messages per short summary", KIND_NUMBER, "Messages between short summaries. Blank uses the server default.",
               placeholder="(server default)", group="Summarization"),
        _field("messagesPerLongSummary", "Messages per long summary", KIND_NUMBER, "Messages between long summaries. Blank uses the server default.",
               placeholder="(server default)", group="Summarization"),
        _field("dreams", "Dream on session end", KIND_BOOL, "Schedule a Honcho dream (memory consolidation + peer card update) when a session ends.",
               default="true", group="Summarization"),
