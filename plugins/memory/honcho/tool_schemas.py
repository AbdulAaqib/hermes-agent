...
PROFILE_SCHEMA = _tool(
    ...
    "Cards hold at most 40 facts — longer writes keep the first 40, so put the "
    "most load-bearing facts first. Prefix each fact with its category so the "
    "card stays scannable: 'IDENTITY: ' (who they are), 'PREFERENCE: ' (how they "
    "want things done), 'TRAIT: ' (how they tend to behave), 'INSTRUCTION: ' "
...
CONCLUDE_SCHEMA = _tool(
    ...
    {"conclusion": {"type": "string",
     ...
     "level": {"type": "string",
               ...
               "description": "Optional reasoning-level filter, used only when `list` is true. 'explicit' = facts extracted directly from messages; 'deductive'/'inductive'/'contradiction' = derived during dreaming."},
     "peer": {"type": "string",
